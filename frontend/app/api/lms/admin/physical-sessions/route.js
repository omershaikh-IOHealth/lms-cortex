...
const STATUS_QUERY = `
  CASE
    ...
    WHEN ps.scheduled_date = CURRENT_DATE AND CURRENT_TIME < ps.start_time THEN 'scheduled'
    WHEN ps.scheduled_date = CURRENT_DATE AND CURRENT_TIME BETWEEN ps.start_time AND ps.end_time THEN 'ongoing'
    WHEN (ps.scheduled_date < CURRENT_DATE) OR (ps.scheduled_date = CURRENT_DATE AND CURRENT_TIME > ps.end_time)
    THEN CASE
      WHEN EXISTS (
        SELECT 1 FROM lms_physical_enrollments pe2
        WHERE pe2.session_id = ps.id AND pe2.attendance_status IN ('present','absent')
...
export async function GET(request) {
  ...
  try {
    ...
    const result = await pool.query(`
      SELECT ps.*, (${STATUS_QUERY}) AS computed_status,
